 ...
 * 실시간 변경은 WebSocket이 담당한다. 응답 board는 {pages:[{id,shapes}], seq} 형태이며 항상 존재한다(빈 보드는 page "p1" 1장).
...
public class ClassroomWhiteboardController {
    ...
    @Operation(summary = "화이트보드 현재 상태 조회", description = "서버 권위 상태(pages+seq)를 받아 동기화. 입장/재동기화용. 수업 멤버만.")
    ...
    public ResponseEntity<Map<String, Object>> getSnapshot(
        ...
        body.put("board", classroomService.getWhiteboardSnapshot(sessionId, userId));
